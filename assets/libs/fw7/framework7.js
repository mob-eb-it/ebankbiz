            ...
            p.monthHTML = function (date, offset) {
                ...
                for (i = 1; i <= rows; i++) {
                    ...
                    for (j = 1; j <= cols; j++) {
						...
						if (p.params.disabledDays) { // 15/12/2018 - ChuongNV add disable day ({'from': 29, 'to': 31})
                            var tempDate = new Date(dayDate);
                            var maxDayOfMonth = new Date(tempDate.getFullYear(), tempDate.getMonth() + 1, 0).getDate();
                            var fromDisableDay = 0;
                            var toDisableDay = 0;
                            if (p.params.disabledDays.from && 0 < p.params.disabledDays.from && p.params.disabledDays.from < 32) {
                                fromDisableDay = p.params.disabledDays.from > maxDayOfMonth ? maxDayOfMonth + 1 : p.params.disabledDays.from;
                            }
                            if (p.params.disabledDays.to && 0 < p.params.disabledDays.to && p.params.disabledDays.to < 32) {
                                toDisableDay = p.params.disabledDays.to > maxDayOfMonth ? maxDayOfMonth + 1 : p.params.disabledDays.to;
                            }
                            if ((fromDisableDay != 0 && dayNumber >= fromDisableDay) && (toDisableDay != 0 && dayNumber <= toDisableDay)) {
                                disabled = true;
                            }
                        } // 15/12/2018 - End ChuongNV add disable day ({'from': 29, 'to': 31})
                        if (disabled) {
